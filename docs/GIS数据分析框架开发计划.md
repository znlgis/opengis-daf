...
构建统一的 GIS 数据分析与数据质检框架，以"方案驱动"为核心，将分析与质检统一在一套执行引擎之上。项目分三阶段递进：Phase 1 打通端到端核心链路，Phase 2 扩产增强至生产可用，Phase 3 完善生态走向平台化。
...
| 3 | 子方案嵌套是 P3 功能 | `SubPlans` 字段保留，引擎不处理 | 方案模型 M4, 调度 M6 |
...
```
Phase 1 (MVP)              Phase 2 (增强)           Phase 3 (生态)
══════════════             ══════════════           ══════════════
M1  项目工程搭建
M2  适配器层                     ┐
M3  算子池+核心算子    ──────────┤
M4  方案管理层                    │                M17 子方案嵌套
M5  执行引擎层                    ├─ M9  并行调度    M18 REST API
M6  调度引擎层(串联)   ──────────┤  M10 算子扩展     M19 Web流程编辑器
M7  质检能力层         ──────────┤  M11 数据源扩展   M20 异构语言(gRPC)
M8  CLI+集成验收       ──────────┤  M12 输出扩展     M21 插件市场
                                  ├─ M13 质检增强    M22 栅格处理
                                  ├─ M14 安全+审计
                                  ├─ M15 性能优化
                                  └─ M16 模板管理
...
> **总工期预估**：10-14 周（2.5-3.5 个月）
> **目标**：多核算力利用、算子体系扩展、生产级适配器、安全与审计
...
> **总工期预估**：12-20 周（3-5 个月）
> **目标**：平台化、生态化、跨语言扩展
> **备注**：此阶段为概要计划，具体里程碑需在 Phase 2 结束时根据实际进展重新细化
...
- 递归方案解析（深度限制，防止无限嵌套）
- 子方案上下文隔离（独立 ExecutionContext 或子上下文）
- 子方案结果回传到父方案（通过 `BindingType.SubPlan`）
- 循环嵌套检测（方案 A 引用方案 B，方案 B 引用方案 A）
- 集成测试

### M18：REST API 完整实现

**工期**：3 周 · **前置**：M14（认证）、M4（方案管理）、M6（执行）

- ASP.NET Core Minimal API 或 Controller-based API
- 实现设计文档 §11 的全部 API 端点
...
**工期**：4 周 · **前置**：M18（REST API）

- 轻量级 Web 前端（技术栈待定：Blazor WASM / React / Vue）
- 可视化 DAG 编排（拖拽算子→配置参数→连接依赖）
- 方案 JSON 双向同步（编辑器 ↔ JSON）
- 模板预览和快速创建
- 执行进度实时展示

### M20：异构语言支持（gRPC Sidecar）

**工期**：3 周 · **前置**：M3（算子接口稳定）

- gRPC 协议定义（`operator.proto`：输入/输出/参数消息）
- .NET 端 gRPC Server（调用本地算子）
- Python/R 端 gRPC Client 模板（标准化算子开发流程）
- 进程管理与健康检查
- 与 .NET 算子统一的注册和执行体验

### M21：插件市场

**工期**：2 周 · **前置**：M10（算子丰富）、M3（插件机制）

- NuGet 标签体系：识别 `opengis-daf-operator` 标签的 NuGet 包
- 内置插件市场 CLI（`daf marketplace search/install`）
- 社区贡献指南和示例插件模板

### M22：栅格数据处理

**工期**：3 周 · **前置**：M2（适配器层）、M3（算子池）

- 栅格数据源适配器（GeoTIFF、IMG 等，通过 GDAL Raster driver）
- `IRasterOperator` 接口扩展
- 基础栅格算子：裁剪、重采样、波段运算、分区统计
- 栅格与矢量混合分析（如按面统计栅格均值）
...
| T2 | GDAL 在 Linux ARM64 环境下编译/运行问题 | 中 | 中 | P1 目标平台仅 x64，ARM64 在 P2/P3 验证；容器化环境先行测试 |
...
| Phase 2 | 10-14 周 | 3-4 人 | +1 名后端（算子扩展并行） |
| Phase 3 | 12-20 周 | 3-5 人 | +1 名前端（Web 编辑器） |
