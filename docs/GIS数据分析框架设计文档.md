...
| 数据格式读写 | OpenGIS Utils for .NET (GDAL) | .NET 封装 GDAL/OGR，负责空间数据格式读写 |
| 坐标转换 | ProjNet | 配合 NTS 执行 CRS 投影转换 |
...
> 异步流式模式（async/await + Channel + IAsyncEnumerable）是框架强制执行的数据通路模式，所有数据读取、处理和输出均遵循此模式，因此列入技术选型表以确保设计一致性。
